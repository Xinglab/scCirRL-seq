...
def assign_ref_bc1(mp_fetch_set1, in_bam, scrl_ref_bcs, scrl_cand_ref_bc_seq, read_to_trans, trans_to_gene_id_name, scrl_para=scrl_para_class()): #, bc_len, bc_max_ed, umi_len, umi_max_ed):
    ...
    umi_clu_res_dict, umi_clu_res_list, umi_ed_to_top_count_dict, umi_ed_to_conn_count_dict = cu.umi_clustering(scrl_para.log_fn, read_to_trans, trans_to_gene_id_name, bu_res, scrl_para.umi_max_ed)
    res = [bu_reads, bc_eds, out_rs, umi_clu_res_dict, umi_clu_res_list, umi_ed_to_top_count_dict, umi_ed_to_conn_count_dict, n_perfect_in_ref_reads, n_perfect_uniq_to_ref_reads, n_imperfect_in_ref_reads, n_imperfect_uniq_to_ref_reads]
...
def assign_ref_bc(mp_fetch_set, n_total_reads, scrl_ref_bcs, scrl_cand_ref_bc_seq, 
    ...
    umi_ed_to_top_count_dict = dd(lambda: 0) # {to_top_ed: count}
    umi_ed_to_conn_count_dict = dd(lambda: 0) # {to_conn_ed: count}
    n_processed_reads = 0
    n_existing_stars = 0
    ...
    with open(scrl_para.out_bu_fn, 'w') as out_fp, ps.AlignmentFile(scrl_para.long_bam) as in_bam:
        ...
        with ps.AlignmentFile(scrl_para.out_bu_bam, 'wb', header=bam_header_dict) as out_bam:
            for mp_fetch_set1 in mp_fetch_set:
                ...
                bu_reads, bc_eds, out_rs, umi_clu_res_dict, umi_clu_res_list, sub_umi_ed_to_top_count_dict, sub_umi_ed_to_conn_count_dict, sub_n_perfect_in_ref_reads, sub_n_perfect_uniq_to_ref_reads, sub_n_imperfect_in_ref_reads, sub_n_imperfect_uniq_to_ref_reads = out_res

                splice_tags = dd(lambda: 'N') # N or Y
                for read in bu_reads:
                    if read in umi_clu_res_dict:
                        bc, umi, cmpt_trans, cmpt_gene_id, cmpt_gene_names, to_top_ed, to_conn_ed = umi_clu_res_dict[read]
                ...
                for umi_ed, count in sub_umi_ed_to_top_count_dict.items():
                    umi_ed_to_top_count_dict[umi_ed] += count
                for umi_ed, count in sub_umi_ed_to_conn_count_dict.items():
                    umi_ed_to_conn_count_dict[umi_ed] += count
    ...
    umi_ed_to_top_count_str = '\n'.join(['%21d : %d (%.1f%%)' % (k, v, v/n_total_reads*100) for k, v in sorted(umi_ed_to_top_count_dict.items())])
    umi_ed_to_conn_count_str = '\n'.join(['%21d : %d (%.1f%%)' % (k, v, v/n_total_reads*100) for k, v in sorted(umi_ed_to_conn_count_dict.items())])
...
UMI edit distance to rep. : read count\n{umi_ed_to_top_count_str}
UMI edit distance to par. : read count\n{umi_ed_to_conn_count_str}
''')
