# NanoHunter
Long-read single-cell RNA-seq data analysis
...
* [rpy2](https://github.com/rpy2/rpy2) >= 3.5.12
...
##  <a name="consensus"></a>Consensus calling for RCA single-cell long-read data (Optional)
### Input
* RCA long-read fasta/fastq file
* 5' and 3' sequence of RCA library. If splint sequence was used, please split splint sequence into two halves and use them as 5' and 3' sequence. Note that both 5' and 3' sequence need to have orientation of 5'->3', i.e., 3' sequence need to be reverse-complement
...
### 1.1 Input 
* Required:
  * sorted long-read BAM (recommend using [minimap2](https://github.com/lh3/minimap2))
  * updated GTF annotation, generated by [ESPRESSO(>=1.3.1)](https://github.com/Xinglab/espresso)
  * tsv file for compatible isoforms of each read, generated by [ESPRESSO(>=1.3.1)](https://github.com/Xinglab/espresso) 
  * output folder
* Optional:
  * cell barcode list, nanohunter will directly use it to guide the barcode calling, only long reads with cell barcode in the list will be kept
  * annotation GTF file, to retrieve gene names. If not provided, NanoHunter will use gene ids as gene names
  * barcode sequence length (16)
  * max. allowed edit distance between barcode and reference barcode (2)
  * UMI sequence length (12)
  * max. allowed edit distance between PCR-duplicated UMIs (1)
...
* `expression_matrix/`: folder containing 10X format sparse expression matrix at both gene and transcrpt level, can be directly parsed using standard single-cell analysis tools, like [Seurat](https://satijalab.org/seurat/)
...
## <a name="cell_splice"></a>2. Cell type-specific splicing analysis
...
* Optional
  * min. number of reads to keep a transcript for each cell type (10)
  * min. number of transcripts to keep a gene for each cell type (2)
  * list of genes of interest. If provided, all genes will still be processed, but only differentially spliced genes that are in this list will be output (None)
### 2.2 Command
```
...
## <a name="allele_splice"></a>3. Allele-specific splicing analysis
...
### 3.1 Phasing long reads with `whatshap`
* Input
...
### <a name="allele_splice_32">3.2 Identifiy allele-specific splicing
* Input
  * `bc_umi.tsv`: tabular file with barcode/UMI/gene/transcript information, generated by NanoHunter
  * `hap_list.tsv`: tabular file containing haplotype information, generated by [whatshap](https://whatshap.readthedocs.io/en/latest/index.html)
  * `bc_to_cell_type.tsv`: list of barcode and corresponding cell type/cluster, generated by [Seurat](https://satijalab.org/seurat/)/[Azimuth](https://satijalab.github.io/azimuth/articles/run_azimuth_tutorial.html) 
...
### 3.3 Identify disease-associated GWAS SNPs from allele-specific spliced genes
* Input
...
* Output
  ...
  * `*_allele_splice_gwas_parent_term.tsv`: GWAS disease-associated EFO parent term of allele-specific spliced gene 
...
Format of `*_allele_splice_gwas_parent_term.tsv`:
